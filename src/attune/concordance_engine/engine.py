...
PACKS: dict[str, ConditionPack] = {p.name: p for p in (PCOS_PACK, VETERAN_PACK)}
...
class Engine:
    ...
    def assess(self, text: str, day: int, *, classifier_flag: bool = False) -> SafetyVerdict:
        ...
        return assess(
            text, self.pack.escalation, classifier_flag=classifier_flag, amber=finding.concordant
        )
