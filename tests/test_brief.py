...
def test_brief_flags_the_concordant_axes_on_a_flare_day():
    brief = build_brief(VETERAN_PACK, generate(VETERAN_PACK, days=90, seed=2), FLARE_DAY)
...
def test_render_maps_criteria_to_concrete_evidence():
    text = render(build_brief(PCOS_PACK, generate(PCOS_PACK, days=90, seed=1), FLARE_DAY))
